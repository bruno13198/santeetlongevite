...
async function traiterHabitude(habitude) {
  ...
  const resultats = await chercherEtudesEuropePMC(habitude);
  ...
  for (const etude of resultatsUniques) {
    ...
    // À partir d'ici, appel Claude (coûteux) : les deux garde-fous s'appliquent.
...
async function main() {
  console.log(`Paramètres de ce run : OFFSET=${OFFSET}, LIMITE=${LIMITE}, JOURS_VEILLE=${JOURS_VEILLE}, MAX_ANALYSES_PAR_RUN=${MAX_ANALYSES_PAR_RUN}`);
